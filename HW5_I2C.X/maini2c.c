...
int main() {
  ...
  __builtin_disable_interrupts();
  init_expander();              // init MCP223008 chip
  i2c_master_setup();                       // init I2C2, which we use as a master
  __builtin_enable_interrupts();
  
  while(1) {
      unsigned char checkpin;
      checkpin = getExpander();
      if ((checkpin >> 7) == 1 ) {
          setExpander(0,0);     // turn LED off if button isn't pressed
      }
      else { 
          setExpander (0,1);   //LED on if button not pressed
      }
  }
  return 0;
}
